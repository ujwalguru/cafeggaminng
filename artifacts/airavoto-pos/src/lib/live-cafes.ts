...
const OPENING_HOURS_DAY_ORDER = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
...
function parseVersionedOpeningHours(value: unknown): { day: string; time: string }[] {
  ...
  if (!payload || typeof payload !== 'object' || !Array.isArray((payload as any).groups)) return [];
  ...
  for (const day of OPENING_HOURS_DAY_ORDER) {
    const group = groups.find((candidate) => Array.isArray(candidate.days) && candidate.days.some((item) => String(item).toLowerCase() === day.toLowerCase()));
    if (!group) continue;
...
export function liveSnapshotToCafe(snapshot: LiveCafeSnapshot): Cafe {
  ...
  const normalizedHours = normalizeHours(metadata.hours ?? metadata.openingHours ?? metadata.opening_hours);
  return {
    id: snapshot.slug,
    slug: snapshot.slug,
    name: snapshot.name,
    ...
    hoursDisplay: String(metadata.hoursDisplay ?? metadata.hours_display ?? normalizedHours[0]?.time ?? 'Live status'),
