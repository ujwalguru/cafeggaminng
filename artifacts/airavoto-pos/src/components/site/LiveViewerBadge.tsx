...
function formatViewerCount(value: number) {
  ...
  if (value < 1_000_000) {
    const compact = value >= 10_000 ? Math.round(value / 1000) : Math.round(value / 100) / 10;
    return `${compact}k+`;
  }
  const compact = Math.round(value / 100_000) / 10;
  return `${compact}M+`;
}
...
export function LiveViewerBadge({ slug, join = false, compact = false }: { slug: string; join?: boolean; compact?: boolean }) {
  ...
  const formattedCount = formatViewerCount(displayCount);

  return (
    <span className={`inline-flex items-center gap-1.5 ${compact ? 'scale-[0.86] origin-right' : ''}`} aria-label={`${displayCount} live viewers`} title={`${displayCount} live viewers`}>
